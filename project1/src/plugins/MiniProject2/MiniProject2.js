    ...
    MiniProject2.prototype.main = function (callback) {
        ...
        self.loadNodeMap(self.rootNode)
            .then(function (nodes) {
				...
				self.buildTreeInfo(self.rootNode, nodes);
                self.result.setSuccess(true);
                var metaNodeInfoJson = JSON.stringify(self.metaNodeInfo, null, 4);
				var metaTreeJson = JSON.stringify(self.hierarchy, null, 4);
                artifact = self.blobClient.createArtifact('project-data');
                self.logger.info(metaNodeInfoJson);
				 self.logger.info(metaTreeJsonJson);
                return artifact.addFiles({'meta.json': metaNodeInfoJson, 'tree.json': metaTreeJson});
		...
		MiniProject2.prototype.buildMetaInfo = function (root, nodes, indent) {
			...
			if(isMetaNode){
            if(isConnection)
                self.metaNodeInfo.push({name: name, path: myPath, numberOfChildren: childrenPaths.length, connection:connectStr, base: self.getMetaType(root); });
            else
                self.metaNodeInfo.push({name: name, path: myPath, numberOfChildren: childrenPaths.length base: self.getMetaType(root);});
		...
		MiniProject2.prototype.buildTreeInfo = function (root, nodes, indent) {
            ...
            for(i = 0; i < childrenPaths.length; i++){
                childNode = nodes[childrenPaths[i]];
				 if(isConnection)
                self.hierarchy.push({name: name, isMeta: isMetaNode, metaType: self.getMetaType(root),  src: src, dst: dst, connection: connectStr, children: self.buildMetaInfo(childNode, nodes, indent + '  '); });
                 else
                self.hierarchy.push({name: name, isMeta: isMetaNode, metaType: self.getMetaType(root), children: self.buildMetaInfo(childNode, nodes, indent + '  '); });
            }
			}
        }
    };
